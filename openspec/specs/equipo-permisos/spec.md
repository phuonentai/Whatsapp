# Spec: equipo-permisos

## Purpose

Defines the consolidated rights-management page ("Equipo y permisos", `?view=access`): three layers — role definitions (read-only, from the Stytch policy), member→role assignment (editable by admin), and modules (plan metadata, summary). The page is a read-only window to the Stytch RBAC policy (runtime SSOT); it never edits the policy in-app.
