...
## Purpose
Defines the repository CI contract: a GitHub Actions workflow (`backend`, `frontend` incl. vitest unit tests, `e2e`) and a GitLab e2e job that must stay aligned with the local `make test-e2e` flow so the two cannot drift.
## Requirements
