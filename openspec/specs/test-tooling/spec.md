...
## Purpose
Defines the offline e2e test tooling contract: mock auth (`AUTH_MOCK_ENABLED` + `X-Test-Org-ID`), the `saas_db_test` seed (orgs/accounts/subscriptions), canonical ports, and the `make test-e2e` one-command runner.
## Requirements
