# Spec: ui-error-recovery

## Purpose

TBD. Defines cross-cutting UI error-recovery behaviour: data-loading views render error and retry states, mutations never fail silently, product flows avoid native browser dialogs, conversation lists show unread inbound indicators, and live updates are announced to assistive technology.

## Requirements

### Requirement: Data-loading views render error and retry states

Every data-loading view (CRM lists, tickets, inbox conversation list, settings sections, knowledge document list) SHALL distinguish loading, error, and success states. When a data query fails, the view SHALL render an inline error state with a retry action instead of an indefinite loading indicator. The system SHALL provide a shared error-state component with a retry button reused across views. Error, empty, and retry copy SHALL render using the typed copy layer in Spanish-first voice across the onboarding, billing, WhatsApp-config, dashboard, agent-settings, and inbox surfaces.
