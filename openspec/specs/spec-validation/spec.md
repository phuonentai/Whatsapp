...
Guarantee the OpenSpec tree stays valid by running `openspec validate --specs` in CI on every push and pull request, so living specs cannot silently regress from the source-of-truth contract.
