...
Defines the pre-login email existence check: the `GET /api/auth/check-email` endpoint contract (read-only local membership lookup, no Stytch call, no session) and the auth page's same-origin API base URL usage for that check.
