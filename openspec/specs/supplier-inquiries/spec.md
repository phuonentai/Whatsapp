...
Defines the procurement capability for Colombian stockless retailers ("venta sin inventario"): an org-scoped suppliers registry (a supplier IS a CRM contact with NIT), a product/SKU catalog, inquiry runs with one metered AI-drafted Spanish message per supplier, durable-outbox fan-out through the circuit-breakered WhatsApp client, structured reply extraction, a deterministic aggregation board, human-approved order placement, and compliance parity (Ley 1581, RBAC, metered AI).
