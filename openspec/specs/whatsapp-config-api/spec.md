...
### Requirement: VerifyChallenge validates verify_token against stored config

The existing `GET /api/v1/webhooks/whatsapp` verification endpoint SHALL validate the `hub.verify_token` query parameter against the stored `verify_token` in `whatsapp.whatsapp_configs`.

#### Scenario: Matching verify_token returns challenge

- **WHEN** a GET request arrives with `hub.mode=subscribe`, a `hub.verify_token` matching an active config, and a `hub.challenge`
- **THEN** the system SHALL return HTTP 200 with the `hub.challenge` as the response body

#### Scenario: Non-matching verify_token is rejected

- **WHEN** a GET request arrives with a `hub.verify_token` that does not match any active config's `verify_token`
- **THEN** the system SHALL return HTTP 403

### Requirement: WhatsApp management routes are registered in API provider

The system SHALL register WhatsApp routes in `internal/api/provider.go` so they are reachable at runtime, alongside the existing module routes (organizations, rbac, billing, documents, cognitive).

#### Scenario: WhatsApp management routes respond to requests
