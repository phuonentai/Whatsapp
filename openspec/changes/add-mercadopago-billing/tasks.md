...
- [x] 3.2 Add SQLC query `GetOrganizationBillingProvider` (returns the provider string)
- [x] 3.3 Add SQLC query `SetOrganizationBillingProvider` (upserts the provider string)
...
- [x] 6.4 Register new routes in `routes.go`

## 7. Frontend: MercadoPago SDK + Server Actions [FE-NEXT]

- [ ] 7.1 Create `lib/mercadopago/config.ts` with env var validation (`MERCADOPAGO_ACCESS_TOKEN`, plan IDs)
- [ ] 7.2 Create `lib/mercadopago/plans.ts` with plan fetching via Go backend (reuse existing plan fetch pattern)
- [ ] 7.3 Create `lib/actions/billing/create-mp-checkout.ts` Server Action — calls Go `POST /api/subscriptions/create-mp-checkout`, returns `init_point` URL
- [ ] 7.4 Create `lib/actions/billing/verify-mp-payment.ts` Server Action — calls Go `POST /api/subscriptions/verify-mp-payment`, returns billing status
- [ ] 7.5 Create `lib/actions/billing/cancel-mp-subscription.ts` Server Action — calls MP `PUT /preapproval/{id}` to cancel

## 8. Frontend: Dual-Provider Checkout UI [FE-NEXT]

- [ ] 8.1 Update `components/billing/plans-modal.tsx` to show two payment options per plan: "International card" (Polar) and "PSE / Nequi / Colombian card" (MP)
- [ ] 8.2 Wire MP option to call `createMercadoPagoCheckout()` server action and redirect to MP `init_point`
- [ ] 8.3 Update post-checkout callback page to detect MP return (check for `payment_id` or `preference_id` query param) and call `verifyMercadoPagoPayment()`
- [ ] 8.4 Update `components/billing/subscription-tab.tsx` to show provider-appropriate actions (MP cancel vs Polar cancel)
- [ ] 8.5 Update `components/billing/subscription-paywall.tsx` to show correct provider info in inactive state

## 9. Environment Configuration

- [ ] 9.1 Add MercadoPago env vars to `example.env`: `MERCADOPAGO_ACCESS_TOKEN`, `MERCADOPAGO_BASE_URL`, `MERCADOPAGO_WEBHOOK_SECRET`, `NEXT_PUBLIC_MERCADOPAGO_PLAN_ID`, `NEXT_PUBLIC_MERCADOPAGO_BUSINESS_PLAN_ID`
- [ ] 9.2 Add MercadoPago env vars to frontend `.env.example`: same vars for Next.js
- [ ] 9.3 Verify config loading in both Go (viper) and Next.js (process.env)

## 10. Integration Testing

- [ ] 10.1 Test MP checkout flow end-to-end with sandbox credentials (create checkout → redirect → verify payment → subscription in DB → paywall pass)
- [ ] 10.2 Test MP webhook receipt and processing (simulate `subscription_authorized`, `subscription_cancelled` webhooks with sandbox)
- [ ] 10.3 Test provider router delegation (org with polar → calls PolarAdapter, org with mercadopago → calls MPAdapter)
- [ ] 10.4 Test Polar checkout still works (regression — existing flow unchanged)
- [ ] 10.5 Test webhook signature verification rejects invalid signatures for both providers
- [ ] 10.6 Test lazy guarding still works with MP provider (expired DB status → `RefreshSubscriptionStatus` → MP API call → update DB)
