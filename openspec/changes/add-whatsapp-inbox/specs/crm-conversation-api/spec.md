...
The system SHALL expose a `GET /crm/conversaciones` endpoint that returns conversations scoped to the authenticated organization, ordered by `last_message_at` descending, with contact display name and last message preview.
...
- **AND** each conversation SHALL include `id`, `contact` (id, phone_number, display_name), `status`, `last_message_at`, `last_message_preview` (content truncated to 100 chars), and `created_at`
...
- **AND** each message SHALL include `id`, `direction` (inbound/outbound), `message_type`, `content`, `status`, `chat_timestamp`, and `created_at`
