...
- [x] 3.4 [FE-NEXT] Commit this change as a separate commit after the pending staged CRM commit lands; do not mix `package.json`/`pnpm-lock.yaml` with the CRM stage — landed `3e57f91` (lint) after `3a398b5` (CRM); `package.json` CRM hunks (dnd-kit, playwright) staged separately via `git add -p`; lockfile landed with CRM commit (typescript-eslint entry spills in, declared in package.json by lint commit — consistent final state)
- [x] 3.5 [FE-NEXT] Record archive decision per governance-workflow — **Archive deferred:** verify lint output in the next frontend change (baseline documented in 3.1), then archive via `/opsx-archive`
