...
- [x] Archive decision resolved (2026-08-12): pending human review completed — delivered site run and reviewed (`/`, `/features`, `/plataforma`, `/pricing`, `/blog`, `/academy`, legal pages: all 200 with expected layout; design consultation + council review `VERDICT.md` STATUS: APPROVED). Archive decision: **ready to archive** — no remaining blockers; technical gates green. (Actual archiving via `/opsx-archive`.)

## Post-review fidelity fixes (2026-08-12) [FE-NEXT]

Review found the delivered homepage diverged from the approved Shuffle reference in hero-mockup fidelity and integrations copy. Fixed:

- [x] Hero mockup aligned to reference: macOS header dots only (label moved to panel), `grid-cols-2 divide-x divide-slate-700`, uppercase panel labels (WhatsApp Chat / Siigo Automático), bubbles upgraded to `text-sm p-3 max-w-[80%]` + document-icon bubble "Factura #001 generada", invoice panel wrapped in inner card (`bg-slate-800 rounded-lg p-4 border`) with "Factura Electrónica" + DIAN badge and real `Enviar Link de Pago` button; floating stat card gained `border-slate-200`. Verify: `pnpm lint` PASS; `pnpm build` PASS; DOM re-check — bubbles 16px, button present
- [x] Integrations strip de-leaked: removed infra names (Polar, Stytch) → `Meta WhatsApp, Siigo, MercadoPago, Nequi, Efecty` (user-facing Colombian rails per `marketing.md`). Verify: DOM re-check — hero lists only the five rails
