...
The system SHALL provide `GET /api/crm/contacts/:id/activities` returning all activities associated with the contact, ordered by `performed_at DESC`.

#### Scenario: Contact timeline with mixed activity types

- **WHEN** a GET request is made to `/api/crm/contacts/1/activities`
...
The system SHALL provide `GET /api/crm/deals/:id/activities` returning all activities associated with the deal, ordered by `performed_at DESC`.

#### Scenario: Deal timeline includes stage changes

- **WHEN** a GET request is made to `/api/crm/deals/1/activities`
...
The system SHALL provide `GET /api/crm/activities` returning all activities for the organization, with optional filters for `type` and `entity_type`/`entity_id`.

#### Scenario: Filter activities by type

- **WHEN** a GET request is made to `/api/crm/activities?type=call`
- **THEN** the system SHALL return only activities with `type = 'call'`

#### Scenario: Filter activities by contact

- **WHEN** a GET request is made to `/api/crm/activities?entity_type=contact&entity_id=1`
