...
- [ ] 7.4 Visual smoke pass ([OPS-GOV]): light + dark mode on shell/auth/dashboard, palette open/close, search results, shortcuts — verify via `pnpm dev` manual pass (no command)
- [ ] 7.5 Run e2e suite ([OPS-GOV]) to confirm no nav regressions. Verify: `pnpm test:e2e`
