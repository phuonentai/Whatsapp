...
- **Member role management** — member list row gains a role control persisting via `PUT /accounts/:id` (`org:manage`), aligning the role vocabulary used by the frontend with the roles accepted by the backend (`admin`, `approver`, `member`).
...
Minimal backend changes: `PUT /organizations` (`UpdateOrganization`) and `PUT /accounts/:id` (`UpdateAccount`) are extended to sync their mutation to Stytch B2B (`Organizations.Update` for display name, `Members.Update` for role) before the local write, keeping both SSOTs in phase; the `UpdateAccount` flow additionally enforces a last-admin guard. No new endpoints, migrations, or SQLC model changes. No local credential storage introduced; identity and roles continue to live in Stytch B2B.
...
- **Backend** (`go-b2b-starter/`): extend `internal/modules/organizations/app/services/organization_service.go` — `UpdateOrganization` calls Stytch `Organizations.Update` (display_name) before the local write; `UpdateAccount` calls Stytch `Members.Update` (role slug) before the local write and enforces a last-admin demotion guard. Existing endpoints consumed unchanged: `PUT /organizations`, `PUT /accounts/:id` (gated `org:manage`), `GET /api/crm/actividades` (gated `contact:view`), verified in `internal/modules/organizations/routes.go` and `internal/modules/crm/routes.go`. Outbound Stytch calls are guarded by a circuit breaker on the shared Stytch client (`internal/platform/stytch`).
...
- No backend endpoint changes, DB migrations, or SQLC model changes (the audit log consumes the existing `GET /api/crm/actividades` endpoint unchanged).
...
- **Stytch tenant policy state**: member role changes are applied through `PUT /accounts/:id`; rollback re-issues the previous role via the same endpoint per affected member. No Stytch org/policy mutations are performed by this change beyond member role updates already exposed by that endpoint.
