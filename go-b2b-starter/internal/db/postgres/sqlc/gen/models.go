// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0

package postgres

import (
	"github.com/jackc/pgx/v5/pgtype"
	pgvector_go "github.com/pgvector/pgvector-go"
)

// Auditoría append-only de decisiones de gobernanza del agente
type AgentAgentAction struct {
	ID                 int32            `json:"id"`
	OrganizationID     int32            `json:"organization_id"`
	FlowID             pgtype.Int4      `json:"flow_id"`
	Action             string           `json:"action"`
	Decision           string           `json:"decision"`
	PolicyInput        []byte           `json:"policy_input"`
	Reasons            []byte           `json:"reasons"`
	ApprovedByMemberID pgtype.Text      `json:"approved_by_member_id"`
	WhatsappMessageID  pgtype.Text      `json:"whatsapp_message_id"`
	RequestID          pgtype.Text      `json:"request_id"`
	CreatedAt          pgtype.Timestamp `json:"created_at"`
}

type AgentAgentSetting struct {
	ID               int32            `json:"id"`
	OrganizationID   int32            `json:"organization_id"`
	Mode             string           `json:"mode"`
	Tone             string           `json:"tone"`
	BrandVoice       pgtype.Text      `json:"brand_voice"`
	AutopilotStart   pgtype.Time      `json:"autopilot_start"`
	AutopilotEnd     pgtype.Time      `json:"autopilot_end"`
	Timezone         string           `json:"timezone"`
	KillSwitch       bool             `json:"kill_switch"`
	MaxDailyMessages int32            `json:"max_daily_messages"`
	ConsentRequired  bool             `json:"consent_required"`
	ConsentTemplate  pgtype.Text      `json:"consent_template"`
	Guardrails       []byte           `json:"guardrails"`
	CreatedAt        pgtype.Timestamp `json:"created_at"`
	UpdatedAt        pgtype.Timestamp `json:"updated_at"`
}

type AgentAgentSuggestion struct {
	ID                 int32            `json:"id"`
	OrganizationID     int32            `json:"organization_id"`
	ConversationID     int32            `json:"conversation_id"`
	ContactID          int32            `json:"contact_id"`
	FlowID             pgtype.Int4      `json:"flow_id"`
	Type               string           `json:"type"`
	Body               pgtype.Text      `json:"body"`
	Metadata           []byte           `json:"metadata"`
	Status             string           `json:"status"`
	Source             string           `json:"source"`
	ApprovedByMemberID pgtype.Text      `json:"approved_by_member_id"`
	WhatsappMessageID  pgtype.Text      `json:"whatsapp_message_id"`
	RequestID          pgtype.Text      `json:"request_id"`
	CreatedAt          pgtype.Timestamp `json:"created_at"`
	UpdatedAt          pgtype.Timestamp `json:"updated_at"`
}

type AgentConversationContext struct {
	ConversationID int32            `json:"conversation_id"`
	Summary        pgtype.Text      `json:"summary"`
	KeyFacts       []byte           `json:"key_facts"`
	DetectedIntent pgtype.Text      `json:"detected_intent"`
	SourceCursor   int64            `json:"source_cursor"`
	ConsentGated   bool             `json:"consent_gated"`
	GeneratedAt    pgtype.Timestamp `json:"generated_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
}

type AgentConversationFlow struct {
	ID             int32            `json:"id"`
	OrganizationID int32            `json:"organization_id"`
	ConversationID int32            `json:"conversation_id"`
	ContactID      int32            `json:"contact_id"`
	Status         string           `json:"status"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
}

// Messages within chat sessions with role (user/assistant/system)
type CognitiveChatMessage struct {
	ID             int32            `json:"id"`
	SessionID      int32            `json:"session_id"`
	Role           string           `json:"role"`
	Content        string           `json:"content"`
	ReferencedDocs []int32          `json:"referenced_docs"`
	TokensUsed     pgtype.Int4      `json:"tokens_used"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
}

// Conversational AI sessions for RAG-based chat
type CognitiveChatSession struct {
	ID             int32            `json:"id"`
	OrganizationID int32            `json:"organization_id"`
	AccountID      int32            `json:"account_id"`
	Title          pgtype.Text      `json:"title"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
}

// Vector embeddings for documents using OpenAI text-embedding-3-small (1536 dimensions)
type CognitiveDocumentEmbedding struct {
	ID             int32 `json:"id"`
	DocumentID     int32 `json:"document_id"`
	OrganizationID int32 `json:"organization_id"`
	// Vector embedding for semantic similarity search
	Embedding      pgvector_go.Vector `json:"embedding"`
	ContentHash    pgtype.Text        `json:"content_hash"`
	ContentPreview pgtype.Text        `json:"content_preview"`
	// Index for chunked documents (0 for single-chunk docs)
	ChunkIndex pgtype.Int4      `json:"chunk_index"`
	CreatedAt  pgtype.Timestamp `json:"created_at"`
	UpdatedAt  pgtype.Timestamp `json:"updated_at"`
}

// Línea de tiempo de actividades CRM (notas, llamadas, correos, etc.)
type CrmActivity struct {
	ID             int32       `json:"id"`
	OrganizationID int32       `json:"organization_id"`
	ContactID      pgtype.Int4 `json:"contact_id"`
	CompanyID      pgtype.Int4 `json:"company_id"`
	DealID         pgtype.Int4 `json:"deal_id"`
	ConversationID pgtype.Int4 `json:"conversation_id"`
	// Tipo de actividad: nota, llamada, correo, reunion, tarea, whatsapp_message, sistema
	Tipo string `json:"tipo"`
	// Asunto o título de la actividad
	Asunto pgtype.Text `json:"asunto"`
	// Contenido detallado de la actividad
	Contenido pgtype.Text `json:"contenido"`
	// Estado (ej: pendiente, completada para tareas)
	Estado pgtype.Text `json:"estado"`
	// Fecha de vencimiento (para tareas)
	FechaVencimiento pgtype.Timestamptz `json:"fecha_vencimiento"`
	// Usuario que realizó la actividad
	RealizadaPor pgtype.Int4 `json:"realizada_por"`
	// Cuándo ocurrió la actividad
	RealizadaEn pgtype.Timestamptz `json:"realizada_en"`
	// Metadatos adicionales (ej: message_id para WhatsApp)
	Metadata  []byte             `json:"metadata"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
}

// Campañas de WhatsApp (v1: draft + snapshot de audiencia)
type CrmCampaign struct {
	ID             int32  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	Nombre         string `json:"nombre"`
	SegmentID      int32  `json:"segment_id"`
	// Estado: draft (no lanzada) o ready (audiencia capturada)
	Status         string             `json:"status"`
	RecipientCount int32              `json:"recipient_count"`
	LaunchedAt     pgtype.Timestamptz `json:"launched_at"`
	// Stytch member_id que creó/lanzó la campaña
	CreatedBy pgtype.Text        `json:"created_by"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
	// Cuerpo del mensaje de la campaña (borrador opcional; nada se envía al crear o lanzar)
	Mensaje pgtype.Text `json:"mensaje"`
}

// Instantánea de audiencia de campaña (bill of materials del envío)
type CrmCampaignRecipient struct {
	ID         int32 `json:"id"`
	CampaignID int32 `json:"campaign_id"`
	ContactID  int32 `json:"contact_id"`
	// Estado por destinatario: pending, sent, failed, skipped
	Status            string             `json:"status"`
	WhatsappMessageID pgtype.Text        `json:"whatsapp_message_id"`
	Error             pgtype.Text        `json:"error"`
	CreatedAt         pgtype.Timestamptz `json:"created_at"`
	UpdatedAt         pgtype.Timestamptz `json:"updated_at"`
}

// Empresas/clientes CRM (no confundir con organizations que son los tenants)
type CrmCompany struct {
	ID             int32  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	Name           string `json:"name"`
	// NIT con dígito de verificación
	Nit pgtype.Text `json:"nit"`
	// Tamaño: microempresa, pequena, mediana, grande
	TipoEmpresa pgtype.Text `json:"tipo_empresa"`
	// Sector industrial
	Sector pgtype.Text `json:"sector"`
	// Ciudad colombiana
	Ciudad pgtype.Text `json:"ciudad"`
	// Departamento colombiano
	Departamento pgtype.Text `json:"departamento"`
	Website      pgtype.Text `json:"website"`
	Phone        pgtype.Text `json:"phone"`
	Address      pgtype.Text `json:"address"`
	Notes        pgtype.Text `json:"notes"`
	Metadata     []byte      `json:"metadata"`
	// Responsable de la empresa
	OwnerAccountID pgtype.Int4        `json:"owner_account_id"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
	UpdatedAt      pgtype.Timestamptz `json:"updated_at"`
}

// Contacts (people who send messages) scoped by organization
type CrmContact struct {
	ID             int32            `json:"id"`
	OrganizationID int32            `json:"organization_id"`
	PhoneNumber    pgtype.Text      `json:"phone_number"`
	DisplayName    pgtype.Text      `json:"display_name"`
	AvatarUrl      pgtype.Text      `json:"avatar_url"`
	Metadata       []byte           `json:"metadata"`
	IsBlocked      bool             `json:"is_blocked"`
	LastMessageAt  pgtype.Timestamp `json:"last_message_at"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
	// Email del contacto (unique per org when non-null)
	Email pgtype.Text `json:"email"`
	// Empresa asociada (FK to crm.companies)
	CompanyID pgtype.Int4 `json:"company_id"`
	// Origen del contacto: whatsapp, manual, import, api
	Source string `json:"source"`
	// Estado comercial: nuevo, contactado, calificado, descalificado, cliente
	LeadStatus string `json:"lead_status"`
	// Cargo del contacto
	JobTitle pgtype.Text `json:"job_title"`
	// Responsable asignado (FK to accounts)
	AssignedTo pgtype.Int4 `json:"assigned_to"`
	// Tipo de documento: CC, NIT, CE, TI, PP
	TipoDocumento pgtype.Text `json:"tipo_documento"`
	// Número de documento de identidad
	NumeroDocumento pgtype.Text `json:"numero_documento"`
	// Estado de consentimiento Ley 1581: none, requested, granted, withdrawn
	ConsentStatus string           `json:"consent_status"`
	ConsentedAt   pgtype.Timestamp `json:"consented_at"`
	// Instagram scoped user id (webhook sender/recipient id)
	InstagramUserID pgtype.Text `json:"instagram_user_id"`
	// Instagram username, backfilled from the Graph API
	InstagramUsername pgtype.Text `json:"instagram_username"`
}

// Conversation threads with contact, supporting 24-hour window matching
type CrmConversation struct {
	ID             int32            `json:"id"`
	OrganizationID int32            `json:"organization_id"`
	ContactID      int32            `json:"contact_id"`
	Status         string           `json:"status"`
	LastMessageAt  pgtype.Timestamp `json:"last_message_at"`
	Metadata       []byte           `json:"metadata"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
	// Messaging channel: whatsapp or instagram
	Channel string `json:"channel"`
	// Miembro asignado vía stytch_member_id (FK lógico a Stytch, patrón crm.tickets; sin tabla local de miembros). NULL = conversación no asignada (cola de leads)
	AssigneeStytchMemberID pgtype.Text `json:"assignee_stytch_member_id"`
}

// Historial append-only de eventos de conversación (re-asignaciones y cambios de estado)
type CrmConversationEvent struct {
	ID             int32  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	ConversationID int32  `json:"conversation_id"`
	EventType      string `json:"event_type"`
	// stytch_member_id del actor (FK lógico a Stytch; sin tabla local de miembros)
	ActorStytchMemberID pgtype.Text `json:"actor_stytch_member_id"`
	// Detalle del evento, p. ej. {"from": <member_id>, "to": <member_id>}
	Payload   []byte             `json:"payload"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
}

// Negocios/oportunidades de venta
type CrmDeal struct {
	ID             int32 `json:"id"`
	OrganizationID int32 `json:"organization_id"`
	// Nombre del negocio
	Nombre     string      `json:"nombre"`
	ContactID  pgtype.Int4 `json:"contact_id"`
	CompanyID  pgtype.Int4 `json:"company_id"`
	PipelineID int32       `json:"pipeline_id"`
	StageID    pgtype.Int4 `json:"stage_id"`
	// Valor en COP (pesos colombianos)
	Monto pgtype.Numeric `json:"monto"`
	// Moneda (default COP)
	Moneda              string      `json:"moneda"`
	FechaCierreEsperada pgtype.Date `json:"fecha_cierre_esperada"`
	// Estado: abierto, ganado, perdido, abandonado
	Estado       string             `json:"estado"`
	Probabilidad pgtype.Int4        `json:"probabilidad"`
	Notas        pgtype.Text        `json:"notas"`
	Metadata     []byte             `json:"metadata"`
	AssignedTo   pgtype.Int4        `json:"assigned_to"`
	CreatedAt    pgtype.Timestamptz `json:"created_at"`
	UpdatedAt    pgtype.Timestamptz `json:"updated_at"`
}

// Relación muchos-a-muchos entre etiquetas y entidades CRM
type CrmEntityTag struct {
	ID         int32              `json:"id"`
	TagID      int32              `json:"tag_id"`
	EntityType string             `json:"entity_type"`
	EntityID   int32              `json:"entity_id"`
	CreatedAt  pgtype.Timestamptz `json:"created_at"`
}

// Individual messages within conversations with WhatsApp metadata
type CrmMessage struct {
	ID                int32            `json:"id"`
	OrganizationID    int32            `json:"organization_id"`
	ConversationID    int32            `json:"conversation_id"`
	ContactID         int32            `json:"contact_id"`
	ProviderMessageID pgtype.Text      `json:"provider_message_id"`
	Direction         string           `json:"direction"`
	MessageType       string           `json:"message_type"`
	Content           pgtype.Text      `json:"content"`
	Status            string           `json:"status"`
	MessageData       []byte           `json:"message_data"`
	ChatTimestamp     pgtype.Timestamp `json:"chat_timestamp"`
	CreatedAt         pgtype.Timestamp `json:"created_at"`
	UpdatedAt         pgtype.Timestamp `json:"updated_at"`
	// Messaging channel: whatsapp or instagram
	Channel string `json:"channel"`
}

// Pipelines de ventas configurables por tenant
type CrmPipeline struct {
	ID             int32 `json:"id"`
	OrganizationID int32 `json:"organization_id"`
	// Nombre del pipeline (ej: Pipeline de Ventas)
	Nombre string `json:"nombre"`
	// Indica si es el pipeline por defecto
	EsPredeterminado bool `json:"es_predeterminado"`
	// Orden de visualización
	Orden     int32              `json:"orden"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
}

// Etapas de cada pipeline de ventas
type CrmPipelineStage struct {
	ID         int32 `json:"id"`
	PipelineID int32 `json:"pipeline_id"`
	// Nombre en español (ej: Prospección, Calificado, etc.)
	Nombre string `json:"nombre"`
	Orden  int32  `json:"orden"`
	// Color hex para kanban (ej: #3B82F6)
	Color pgtype.Text `json:"color"`
	// Probabilidad de cierre (0-100, NULL para etapas de salida)
	Probabilidad   pgtype.Int4        `json:"probabilidad"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
	UpdatedAt      pgtype.Timestamptz `json:"updated_at"`
	OrganizationID int32              `json:"organization_id"`
}

// Segmentos guardados de contactos (audiencia de campañas)
type CrmSegment struct {
	ID             int32  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	Nombre         string `json:"nombre"`
	// JSON array de filtros validados: [{"field","op","value"}] con semántica AND
	FilterSpec []byte `json:"filter_spec"`
	// Stytch member_id que creó el segmento
	CreatedBy pgtype.Text        `json:"created_by"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
}

// Etiquetas para segmentar contactos, empresas y negocios
type CrmTag struct {
	ID             int32 `json:"id"`
	OrganizationID int32 `json:"organization_id"`
	// Nombre de la etiqueta
	Nombre string `json:"nombre"`
	// Color hex (ej: #F59E0B)
	Color     pgtype.Text        `json:"color"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
}

// Tickets de soporte (módulo vendible)
type CrmTicket struct {
	ID             int32       `json:"id"`
	OrganizationID int32       `json:"organization_id"`
	ContactID      pgtype.Int4 `json:"contact_id"`
	ConversationID pgtype.Int4 `json:"conversation_id"`
	Title          string      `json:"title"`
	Description    pgtype.Text `json:"description"`
	Status         string      `json:"status"`
	Priority       string      `json:"priority"`
	Tags           []byte      `json:"tags"`
	// Miembro asignado vía stytch_member_id (sin tabla local de miembros)
	AssigneeStytchMemberID pgtype.Text `json:"assignee_stytch_member_id"`
	// Fecha límite SLA calculada según config del módulo (por prioridad)
	SlaDueAt  pgtype.Timestamptz `json:"sla_due_at"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
}

// Historial append-only de eventos del ticket
type CrmTicketEvent struct {
	ID                  int32              `json:"id"`
	TicketID            int32              `json:"ticket_id"`
	EventType           string             `json:"event_type"`
	ActorStytchMemberID pgtype.Text        `json:"actor_stytch_member_id"`
	Payload             []byte             `json:"payload"`
	CreatedAt           pgtype.Timestamptz `json:"created_at"`
}

// Stores uploaded documents (PDFs) with extracted text for RAG
type DocumentsDocument struct {
	ID             int32  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	FileAssetID    int32  `json:"file_asset_id"`
	Title          string `json:"title"`
	FileName       string `json:"file_name"`
	ContentType    string `json:"content_type"`
	FileSize       int64  `json:"file_size"`
	// Text extracted from PDF using OCR or direct parsing
	ExtractedText pgtype.Text `json:"extracted_text"`
	// Processing status: pending, processing, processed, failed
	Status    string           `json:"status"`
	Metadata  []byte           `json:"metadata"`
	CreatedAt pgtype.Timestamp `json:"created_at"`
	UpdatedAt pgtype.Timestamp `json:"updated_at"`
	// Document visibility ACL: workspace (all org members) | admin_only (org:manage only).
	Visibility string `json:"visibility"`
}

// Stores potential duplicate resources found via vector similarity and LLM adjudication
type DuplicateCandidate struct {
	ID                  int32 `json:"id"`
	ResourceID          int32 `json:"resource_id"`
	CandidateResourceID int32 `json:"candidate_resource_id"`
	// Cosine similarity score from pgvector (0.0000 = completely different, 1.0000 = identical)
	SimilarityScore pgtype.Numeric `json:"similarity_score"`
	// How the duplicate was detected: exact_match (similarity >= 0.95) or llm_adjudicated (similarity >= 0.85, confirmed by LLM)
	DetectionMethod  string           `json:"detection_method"`
	ConfidenceLevel  pgtype.Text      `json:"confidence_level"`
	LlmReason        pgtype.Text      `json:"llm_reason"`
	LlmSimilarFields []byte           `json:"llm_similar_fields"`
	LlmResponse      []byte           `json:"llm_response"`
	OrganizationID   int32            `json:"organization_id"`
	Status           pgtype.Text      `json:"status"`
	CreatedAt        pgtype.Timestamp `json:"created_at"`
	UpdatedAt        pgtype.Timestamp `json:"updated_at"`
}

// Example module demonstrating Clean Architecture patterns with file uploads, OCR/LLM processing, RBAC, approval workflows, and multi-tenancy
type ExampleResource struct {
	ID             int32       `json:"id"`
	ResourceNumber string      `json:"resource_number"`
	Title          string      `json:"title"`
	Description    pgtype.Text `json:"description"`
	StatusID       int16       `json:"status_id"`
	FileID         pgtype.Int4 `json:"file_id"`
	// Raw OCR-extracted text and metadata
	ExtractedData []byte `json:"extracted_data"`
	// LLM-processed structured data
	ProcessedData []byte `json:"processed_data"`
	// AI confidence score between 0 and 1
	Confidence         pgtype.Numeric `json:"confidence"`
	OrganizationID     int32          `json:"organization_id"`
	CreatedByAccountID pgtype.Int4    `json:"created_by_account_id"`
	// Workflow status: pending, approved, rejected
	ApprovalStatus        pgtype.Text      `json:"approval_status"`
	ApprovalAssignedToID  pgtype.Int4      `json:"approval_assigned_to_id"`
	ApprovalActionTakerID pgtype.Int4      `json:"approval_action_taker_id"`
	ApprovalNotes         pgtype.Text      `json:"approval_notes"`
	Metadata              []byte           `json:"metadata"`
	IsActive              bool             `json:"is_active"`
	CreatedAt             pgtype.Timestamp `json:"created_at"`
	UpdatedAt             pgtype.Timestamp `json:"updated_at"`
}

type FileManagerFileAsset struct {
	ID               int32              `json:"id"`
	FileName         string             `json:"file_name"`
	OriginalFileName string             `json:"original_file_name"`
	StoragePath      string             `json:"storage_path"`
	BucketName       string             `json:"bucket_name"`
	FileSize         int64              `json:"file_size"`
	MimeType         string             `json:"mime_type"`
	FileCategoryID   int16              `json:"file_category_id"`
	FileContextID    int16              `json:"file_context_id"`
	IsPublic         pgtype.Bool        `json:"is_public"`
	EntityType       pgtype.Text        `json:"entity_type"`
	EntityID         pgtype.Int4        `json:"entity_id"`
	Purpose          pgtype.Text        `json:"purpose"`
	Metadata         []byte             `json:"metadata"`
	CreatedAt        pgtype.Timestamptz `json:"created_at"`
	UpdatedAt        pgtype.Timestamptz `json:"updated_at"`
	OrganizationID   int32              `json:"organization_id"`
}

type FileManagerFileCategory struct {
	ID           int16  `json:"id"`
	Name         string `json:"name"`
	MaxSizeBytes int64  `json:"max_size_bytes"`
}

type FileManagerFileContext struct {
	ID   int16  `json:"id"`
	Name string `json:"name"`
}

// Registro de corridas de importación de clientes (preview/confirm/delta)
type InvoicingImportRun struct {
	ID             int64              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	Kind           string             `json:"kind"`
	Counts         []byte             `json:"counts"`
	Error          pgtype.Text        `json:"error"`
	PulledAt       pgtype.Timestamptz `json:"pulled_at"`
}

// Facturas electrónicas locales (rail: Siigo/DIAN)
type InvoicingInvoice struct {
	ID             int64       `json:"id"`
	OrganizationID int32       `json:"organization_id"`
	DealID         pgtype.Int4 `json:"deal_id"`
	// ID de la factura en el proveedor (Siigo)
	ExternalID pgtype.Text `json:"external_id"`
	// CUFE emitido por DIAN (opaco; se reenvía tal cual del proveedor)
	Cufe pgtype.Text `json:"cufe"`
	// pending | valid | invalid | errored
	Status   string         `json:"status"`
	PdfUrl   pgtype.Text    `json:"pdf_url"`
	Amount   pgtype.Numeric `json:"amount"`
	Currency string         `json:"currency"`
	// Último estado por el que se notificó al contacto por WhatsApp (una notificación por transición)
	NotifiedStatus pgtype.Text        `json:"notified_status"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
	UpdatedAt      pgtype.Timestamptz `json:"updated_at"`
}

// Conexión de facturación por organización (rail: Siigo)
type InvoicingOrgConnection struct {
	OrganizationID int32  `json:"organization_id"`
	Provider       string `json:"provider"`
	// none | awaiting_setup | connected | numeracion_ok | sandbox_ok | live | paused | invoicing_disabled
	Status string `json:"status"`
	// client_id cifrado (AES-256-GCM, nonce+tag incluidos)
	ClientIDEnc []byte `json:"client_id_enc"`
	// client_secret cifrado (AES-256-GCM, nonce+tag incluidos)
	ClientSecretEnc  []byte             `json:"client_secret_enc"`
	Nit              pgtype.Text        `json:"nit"`
	SiigoCompanyName pgtype.Text        `json:"siigo_company_name"`
	LastError        pgtype.Text        `json:"last_error"`
	PausedAt         pgtype.Timestamptz `json:"paused_at"`
	CreatedAt        pgtype.Timestamptz `json:"created_at"`
	UpdatedAt        pgtype.Timestamptz `json:"updated_at"`
}

// Confirmación de numeración (resolución DIAN) por organización; snapshot en el momento de confirmación
type InvoicingOrgNumeration struct {
	OrganizationID int32 `json:"organization_id"`
	// auto: Siigo asigna consecutivo; manual: la plataforma obtiene el próximo número (reservado)
	Mode         string             `json:"mode"`
	ResolutionID pgtype.Text        `json:"resolution_id"`
	Prefijo      pgtype.Text        `json:"prefijo"`
	NextNumber   pgtype.Text        `json:"next_number"`
	ConfirmedAt  pgtype.Timestamptz `json:"confirmed_at"`
	CreatedAt    pgtype.Timestamptz `json:"created_at"`
	UpdatedAt    pgtype.Timestamptz `json:"updated_at"`
}

// Catálogo de módulos vendibles del producto
type ModulesModule struct {
	ID          int32       `json:"id"`
	Key         string      `json:"key"`
	Name        string      `json:"name"`
	Description pgtype.Text `json:"description"`
	// Feature keys concedidos por el módulo (ej: ["tickets_module"])
	GrantedFeatures []byte `json:"granted_features"`
	// Keys de otros módulos requeridos
	Requires []byte `json:"requires"`
	// Esquema JSON del config por organización
	ConfigSchema []byte `json:"config_schema"`
	// Módulo solo para el vendor (org #0); oculto a tenants
	IsInternal bool               `json:"is_internal"`
	IsActive   bool               `json:"is_active"`
	CreatedAt  pgtype.Timestamptz `json:"created_at"`
}

// Estado y configuración de módulos por organización
type ModulesOrganizationModule struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	ModuleKey      string             `json:"module_key"`
	Config         []byte             `json:"config"`
	EnabledAt      pgtype.Timestamptz `json:"enabled_at"`
}

// Playbooks aplicados por organización
type ModulesOrganizationPlaybook struct {
	ID               int32              `json:"id"`
	OrganizationID   int32              `json:"organization_id"`
	PlaybookKey      string             `json:"playbook_key"`
	SeededPipelineID pgtype.Int4        `json:"seeded_pipeline_id"`
	AppliedAt        pgtype.Timestamptz `json:"applied_at"`
}

// Catálogo de playbooks verticales (procedimientos listos para usar)
type ModulesPlaybook struct {
	ID          int32       `json:"id"`
	Key         string      `json:"key"`
	Name        string      `json:"name"`
	Vertical    string      `json:"vertical"`
	Description pgtype.Text `json:"description"`
	// Keys de módulos que deben estar habilitados (vacío = sin requisitos)
	RequiresModules []byte `json:"requires_modules"`
	// Payload semilla: {pipeline:{nombre,etapas[]}, tags[], module_configs{}, guiones[{id,titulo,mensaje}]}
	Payload   []byte             `json:"payload"`
	IsActive  bool               `json:"is_active"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
}

// User accounts within organizations
type OrganizationsAccount struct {
	ID             int32  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	Email          string `json:"email"`
	FullName       string `json:"full_name"`
	// Stytch member identifier (member_xxx)
	StytchMemberID pgtype.Text `json:"stytch_member_id"`
	// Stytch role identifier assigned to the member
	StytchRoleID pgtype.Text `json:"stytch_role_id"`
	// Human-readable Stytch role slug assigned to the member
	StytchRoleSlug pgtype.Text `json:"stytch_role_slug"`
	// Whether Stytch reports the member email as verified
	StytchEmailVerified bool `json:"stytch_email_verified"`
	// Last known role for business logic (e.g., owner, reviewer, employee)
	Role        string           `json:"role"`
	Status      string           `json:"status"`
	LastLoginAt pgtype.Timestamp `json:"last_login_at"`
	CreatedAt   pgtype.Timestamp `json:"created_at"`
	UpdatedAt   pgtype.Timestamp `json:"updated_at"`
}

// Organizations (tenants) in the system
type OrganizationsOrganization struct {
	ID int32 `json:"id"`
	// URL-friendly unique identifier for organization
	Slug   string `json:"slug"`
	Name   string `json:"name"`
	Status string `json:"status"`
	// Stytch organization identifier (org_xxx)
	StytchOrgID pgtype.Text `json:"stytch_org_id"`
	// Optional Stytch connection or project identifier associated with the organization
	StytchConnectionID pgtype.Text `json:"stytch_connection_id"`
	// Optional Stytch connection name associated with the organization
	StytchConnectionName pgtype.Text      `json:"stytch_connection_name"`
	CreatedAt            pgtype.Timestamp `json:"created_at"`
	UpdatedAt            pgtype.Timestamp `json:"updated_at"`
	// Billing provider preference for this organization. NULL/polar = Polar.sh, mercadopago = MercadoPago
	BillingProvider pgtype.Text `json:"billing_provider"`
}

// Durable event outbox: events committed with their source transaction and dispatched asynchronously with retry and dead-letter
type OutboxEvent struct {
	ID             int64       `json:"id"`
	EventType      string      `json:"event_type"`
	Payload        []byte      `json:"payload"`
	OrganizationID pgtype.Int4 `json:"organization_id"`
	// pending | dispatched | dead_letter
	Status   string `json:"status"`
	Attempts int32  `json:"attempts"`
	// Backoff timestamp for retry scheduling; dispatcher claims rows where next_attempt_at <= now()
	NextAttemptAt pgtype.Timestamptz `json:"next_attempt_at"`
	LastError     pgtype.Text        `json:"last_error"`
	CreatedAt     pgtype.Timestamptz `json:"created_at"`
	DispatchedAt  pgtype.Timestamptz `json:"dispatched_at"`
}

// Pagos de clientes del SMB (rail: MercadoPago, one-shot payment links)
type PaymentsClientPayment struct {
	ID             int64       `json:"id"`
	OrganizationID int32       `json:"organization_id"`
	DealID         int32       `json:"deal_id"`
	InvoiceID      pgtype.Int4 `json:"invoice_id"`
	// Monto base en COP (por cobrar del SMB)
	AmountCop int64 `json:"amount_cop"`
	// Comisión de plataforma en COP (marcado sobre amount_cop)
	CommissionCop int64  `json:"commission_cop"`
	Currency      string `json:"currency"`
	// pending | paid | failed | expired
	Status         string             `json:"status"`
	MpPreferenceID pgtype.Text        `json:"mp_preference_id"`
	MpPaymentID    pgtype.Text        `json:"mp_payment_id"`
	PaidAt         pgtype.Timestamptz `json:"paid_at"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
}

// Auditoría append-only de procurement (alta, consentimiento, órdenes, bloqueos)
type ProcurementAuditLog struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	EntityType     string             `json:"entity_type"`
	EntityID       pgtype.Int4        `json:"entity_id"`
	Action         string             `json:"action"`
	Decision       string             `json:"decision"`
	Reason         pgtype.Text        `json:"reason"`
	MemberID       pgtype.Text        `json:"member_id"`
	Metadata       []byte             `json:"metadata"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
}

type ProcurementInquiryRecipient struct {
	ID                int32              `json:"id"`
	OrganizationID    int32              `json:"organization_id"`
	RunID             int32              `json:"run_id"`
	SupplierID        int32              `json:"supplier_id"`
	ContactID         int32              `json:"contact_id"`
	DraftedMessage    pgtype.Text        `json:"drafted_message"`
	Status            string             `json:"status"`
	ProviderMessageID pgtype.Text        `json:"provider_message_id"`
	SentAt            pgtype.Timestamptz `json:"sent_at"`
	AnsweredAt        pgtype.Timestamptz `json:"answered_at"`
	FollowupCount     int32              `json:"followup_count"`
	CreatedAt         pgtype.Timestamptz `json:"created_at"`
	UpdatedAt         pgtype.Timestamptz `json:"updated_at"`
}

type ProcurementInquiryResponse struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	RecipientID    int32              `json:"recipient_id"`
	RawMessageID   string             `json:"raw_message_id"`
	Extracted      []byte             `json:"extracted"`
	Resumen        pgtype.Text        `json:"resumen"`
	Confidence     pgtype.Float8      `json:"confidence"`
	RequiereHumano bool               `json:"requiere_humano"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
}

// Ejecuciones de cotización (flujo de procurement)
type ProcurementInquiryRun struct {
	ID                int32              `json:"id"`
	OrganizationID    int32              `json:"organization_id"`
	Status            string             `json:"status"`
	Source            string             `json:"source"`
	ScheduleRef       pgtype.Int8        `json:"schedule_ref"`
	Nota              pgtype.Text        `json:"nota"`
	CreatedByMemberID pgtype.Text        `json:"created_by_member_id"`
	SentAt            pgtype.Timestamptz `json:"sent_at"`
	CompletedAt       pgtype.Timestamptz `json:"completed_at"`
	CreatedAt         pgtype.Timestamptz `json:"created_at"`
	UpdatedAt         pgtype.Timestamptz `json:"updated_at"`
}

type ProcurementOrder struct {
	ID                int32              `json:"id"`
	OrganizationID    int32              `json:"organization_id"`
	RunID             int32              `json:"run_id"`
	SupplierID        int32              `json:"supplier_id"`
	ContactID         int32              `json:"contact_id"`
	NegocioID         pgtype.Int4        `json:"negocio_id"`
	Status            string             `json:"status"`
	Items             []byte             `json:"items"`
	Notes             pgtype.Text        `json:"notes"`
	ConfirmMessage    pgtype.Text        `json:"confirm_message"`
	BlockedReason     pgtype.Text        `json:"blocked_reason"`
	CreatedByMemberID pgtype.Text        `json:"created_by_member_id"`
	CreatedAt         pgtype.Timestamptz `json:"created_at"`
	UpdatedAt         pgtype.Timestamptz `json:"updated_at"`
}

type ProcurementProduct struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	Name           string             `json:"name"`
	Sku            string             `json:"sku"`
	Unit           string             `json:"unit"`
	IsActive       bool               `json:"is_active"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
	UpdatedAt      pgtype.Timestamptz `json:"updated_at"`
}

// Programaciones de cotizaciones (hora + días de la semana, zona horaria de la organización)
type ProcurementSchedule struct {
	ID                  int32              `json:"id"`
	OrganizationID      int32              `json:"organization_id"`
	Name                string             `json:"name"`
	RunTime             pgtype.Time        `json:"run_time"`
	DaysOfWeek          []int16            `json:"days_of_week"`
	Note                pgtype.Text        `json:"note"`
	IsActive            bool               `json:"is_active"`
	NextRunAt           pgtype.Timestamptz `json:"next_run_at"`
	LastRunAt           pgtype.Timestamptz `json:"last_run_at"`
	LastRunOccurrenceAt pgtype.Timestamptz `json:"last_run_occurrence_at"`
	CreatedAt           pgtype.Timestamptz `json:"created_at"`
	UpdatedAt           pgtype.Timestamptz `json:"updated_at"`
}

// Configuración de recordatorios por organización (plazo, recordatorios máximos, plantilla)
type ProcurementScheduleFollowup struct {
	ID              int32              `json:"id"`
	OrganizationID  int32              `json:"organization_id"`
	Enabled         bool               `json:"enabled"`
	DeadlineHours   int32              `json:"deadline_hours"`
	MaxNudges       int32              `json:"max_nudges"`
	MessageTemplate string             `json:"message_template"`
	CreatedAt       pgtype.Timestamptz `json:"created_at"`
	UpdatedAt       pgtype.Timestamptz `json:"updated_at"`
}

// Productos incluidos en una programación
type ProcurementScheduleProduct struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	ScheduleID     int32              `json:"schedule_id"`
	ProductID      int32              `json:"product_id"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
}

// Proveedores incluidos en una programación
type ProcurementScheduleSupplier struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	ScheduleID     int32              `json:"schedule_id"`
	SupplierID     int32              `json:"supplier_id"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
}

// Proveedores: un proveedor ES un contacto CRM (NIT persona jurídica)
type ProcurementSupplier struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	ContactID      int32              `json:"contact_id"`
	Nit            string             `json:"nit"`
	DeliveryDays   pgtype.Int4        `json:"delivery_days"`
	MinOrderAmount pgtype.Numeric     `json:"min_order_amount"`
	Notes          pgtype.Text        `json:"notes"`
	IsActive       bool               `json:"is_active"`
	CreatedAt      pgtype.Timestamptz `json:"created_at"`
	UpdatedAt      pgtype.Timestamptz `json:"updated_at"`
}

// Stores vector embeddings for resources using OpenAI text-embedding-3-small (1536 dimensions)
type ResourceEmbedding struct {
	ID         int32 `json:"id"`
	ResourceID int32 `json:"resource_id"`
	// Vector embedding for semantic similarity search (1536 dimensions from OpenAI)
	Embedding      pgvector_go.Vector `json:"embedding"`
	OrganizationID int32              `json:"organization_id"`
	// SHA-256 hash of normalized content for exact duplicate detection
	ContentHash    pgtype.Text      `json:"content_hash"`
	ContentPreview pgtype.Text      `json:"content_preview"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
}

// Totales corrientes de consumo de tokens/credits IA por organización y periodo
type SubscriptionBillingAiUsage struct {
	ID              int32              `json:"id"`
	OrganizationID  int32              `json:"organization_id"`
	PeriodStart     pgtype.Timestamptz `json:"period_start"`
	PeriodEnd       pgtype.Timestamptz `json:"period_end"`
	TokensInput     int64              `json:"tokens_input"`
	TokensOutput    int64              `json:"tokens_output"`
	TokensEmbedding int64              `json:"tokens_embedding"`
	// Créditos consumidos en el periodo (conversión token->crédito)
	CreditsUsed int32              `json:"credits_used"`
	CreatedAt   pgtype.Timestamptz `json:"created_at"`
	UpdatedAt   pgtype.Timestamptz `json:"updated_at"`
}

// Auditoría append-only de consumo IA por organización
type SubscriptionBillingAiUsageEvent struct {
	ID              int32  `json:"id"`
	OrganizationID  int32  `json:"organization_id"`
	Feature         string `json:"feature"`
	Model           string `json:"model"`
	TokensInput     int64  `json:"tokens_input"`
	TokensOutput    int64  `json:"tokens_output"`
	TokensEmbedding int64  `json:"tokens_embedding"`
	CreditsConsumed int32  `json:"credits_consumed"`
	// Identificador idempotente (ON CONFLICT DO NOTHING evita dobles registros)
	RequestID string             `json:"request_id"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
}

// Tracks usage quotas per organization for fast quota checks
type SubscriptionBillingQuotaTracking struct {
	ID             int32            `json:"id"`
	OrganizationID int32            `json:"organization_id"`
	MaxSeats       pgtype.Int4      `json:"max_seats"`
	PeriodStart    pgtype.Timestamp `json:"period_start"`
	PeriodEnd      pgtype.Timestamp `json:"period_end"`
	LastSyncedAt   pgtype.Timestamp `json:"last_synced_at"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
	// Remaining invoices in current billing period (decremented on use)
	InvoiceCount int32 `json:"invoice_count"`
	// Créditos IA disponibles en el periodo (metadata de producto / meter grant ai.tokens)
	AiCreditsMax int32 `json:"ai_credits_max"`
}

// Stores subscription details from Polar, synced via webhooks
type SubscriptionBillingSubscription struct {
	ID             int32 `json:"id"`
	OrganizationID int32 `json:"organization_id"`
	// Polar customer ID (maps to organization via stytch_org_id)
	ExternalCustomerID string           `json:"external_customer_id"`
	SubscriptionID     string           `json:"subscription_id"`
	SubscriptionStatus string           `json:"subscription_status"`
	ProductID          string           `json:"product_id"`
	ProductName        pgtype.Text      `json:"product_name"`
	PlanName           pgtype.Text      `json:"plan_name"`
	CurrentPeriodStart pgtype.Timestamp `json:"current_period_start"`
	CurrentPeriodEnd   pgtype.Timestamp `json:"current_period_end"`
	CancelAtPeriodEnd  pgtype.Bool      `json:"cancel_at_period_end"`
	CanceledAt         pgtype.Timestamp `json:"canceled_at"`
	CreatedAt          pgtype.Timestamp `json:"created_at"`
	UpdatedAt          pgtype.Timestamp `json:"updated_at"`
	Metadata           []byte           `json:"metadata"`
}

// Maps Instagram business ig_user_id to organizations with webhook secrets and messaging tokens
type WhatsappInstagramConfig struct {
	ID             int32              `json:"id"`
	OrganizationID int32              `json:"organization_id"`
	IgUserID       string             `json:"ig_user_id"`
	IgUsername     pgtype.Text        `json:"ig_username"`
	FbPageID       pgtype.Text        `json:"fb_page_id"`
	AccessToken    string             `json:"access_token"`
	TokenExpiresAt pgtype.Timestamptz `json:"token_expires_at"`
	WebhookSecret  string             `json:"webhook_secret"`
	VerifyToken    string             `json:"verify_token"`
	ApiVersion     string             `json:"api_version"`
	GraphApiUrl    string             `json:"graph_api_url"`
	IsActive       bool               `json:"is_active"`
	Metadata       []byte             `json:"metadata"`
	CreatedAt      pgtype.Timestamp   `json:"created_at"`
	UpdatedAt      pgtype.Timestamp   `json:"updated_at"`
}

// Raw Instagram webhook payloads for audit and replay
type WhatsappInstagramWebhookLog struct {
	ID             int32            `json:"id"`
	OrganizationID pgtype.Int4      `json:"organization_id"`
	Status         string           `json:"status"`
	EventType      pgtype.Text      `json:"event_type"`
	IgUserID       pgtype.Text      `json:"ig_user_id"`
	RawHeaders     []byte           `json:"raw_headers"`
	RawBody        []byte           `json:"raw_body"`
	ErrorMessage   pgtype.Text      `json:"error_message"`
	ProcessedAt    pgtype.Timestamp `json:"processed_at"`
	DeliveryKey    pgtype.Text      `json:"delivery_key"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
}

// Embedded signup provisioning state per organization (one row per org)
type WhatsappSignupFlow struct {
	ID             int64 `json:"id"`
	OrganizationID int32 `json:"organization_id"`
	// exchanging | registering | verifying | connected | failed
	Status string `json:"status"`
	// Current sub-step for mid-flow recovery after token code consumption
	Step pgtype.Text `json:"step"`
	// Meta/backend error code on terminal failure
	ErrorCode  pgtype.Text `json:"error_code"`
	RetryCount int32       `json:"retry_count"`
	// Provisioning metadata: waba_id, phone_number_id, coexistence, ticket_id
	Metadata  []byte             `json:"metadata"`
	CreatedAt pgtype.Timestamptz `json:"created_at"`
	UpdatedAt pgtype.Timestamptz `json:"updated_at"`
}

// Plantillas de mensaje de WhatsApp por organización (autoría local; Meta aprueba el envío)
type WhatsappTemplate struct {
	ID             int64  `json:"id"`
	OrganizationID int32  `json:"organization_id"`
	Name           string `json:"name"`
	Category       string `json:"category"`
	Language       string `json:"language"`
	Body           string `json:"body"`
	// Número de parámetros {{N}} en body (calculado al crear/editar)
	ParamCount int32 `json:"param_count"`
	// draft | submitted | approved | rejected | paused
	Status          string             `json:"status"`
	MetaTemplateID  pgtype.Text        `json:"meta_template_id"`
	RejectionReason pgtype.Text        `json:"rejection_reason"`
	IsActive        bool               `json:"is_active"`
	CreatedAt       pgtype.Timestamptz `json:"created_at"`
	UpdatedAt       pgtype.Timestamptz `json:"updated_at"`
}

// Raw webhook payloads for audit and replay
type WhatsappWebhookLog struct {
	ID int32 `json:"id"`
	// Resolved organization for the webhook; NULL when the failure occurs before organization resolution (e.g., unknown phone_number_id)
	OrganizationID pgtype.Int4      `json:"organization_id"`
	Status         string           `json:"status"`
	EventType      pgtype.Text      `json:"event_type"`
	PhoneNumberID  pgtype.Text      `json:"phone_number_id"`
	RawHeaders     []byte           `json:"raw_headers"`
	RawBody        []byte           `json:"raw_body"`
	ErrorMessage   pgtype.Text      `json:"error_message"`
	ProcessedAt    pgtype.Timestamp `json:"processed_at"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	// Stable provider delivery key (entry id + message ids) used for at-least-once dedup; NULL for legacy/failed-before-resolution rows
	DeliveryKey pgtype.Text `json:"delivery_key"`
}

// Maps WhatsApp phone_number_id to organizations with webhook secrets
type WhatsappWhatsappConfig struct {
	ID             int32            `json:"id"`
	OrganizationID int32            `json:"organization_id"`
	PhoneNumberID  string           `json:"phone_number_id"`
	BusinessPhone  string           `json:"business_phone"`
	WebhookSecret  string           `json:"webhook_secret"`
	VerifyToken    string           `json:"verify_token"`
	AppID          pgtype.Text      `json:"app_id"`
	IsActive       bool             `json:"is_active"`
	Metadata       []byte           `json:"metadata"`
	CreatedAt      pgtype.Timestamp `json:"created_at"`
	UpdatedAt      pgtype.Timestamp `json:"updated_at"`
	// WhatsApp Business Account ID for outbound API calls
	WabaID pgtype.Text `json:"waba_id"`
	// Permanent access token for WhatsApp Cloud API authentication
	AccessToken pgtype.Text `json:"access_token"`
	// WhatsApp Cloud API version (default: v21.0)
	ApiVersion string `json:"api_version"`
	// Base URL for Graph API (default: https://graph.facebook.com)
	GraphApiUrl string `json:"graph_api_url"`
}
