...
const getOrganizationBillingProvider = `-- name: GetOrganizationBillingProvider :one
SELECT COALESCE(billing_provider, 'polar') AS billing_provider
FROM organizations.organizations
WHERE id = $1
`

func (q *Queries) GetOrganizationBillingProvider(ctx context.Context, id int32) (string, error) {
	row := q.db.QueryRow(ctx, getOrganizationBillingProvider, id)
	var billing_provider string
	err := row.Scan(&billing_provider)
	return billing_provider, err
}
...
const setOrganizationBillingProvider = `-- name: SetOrganizationBillingProvider :one
...
type SetOrganizationBillingProviderParams struct {
	ID              int32       `json:"id"`
	BillingProvider pgtype.Text `json:"billing_provider"`
}

func (q *Queries) SetOrganizationBillingProvider(ctx context.Context, arg SetOrganizationBillingProviderParams) (string, error) {
	row := q.db.QueryRow(ctx, setOrganizationBillingProvider, arg.ID, arg.BillingProvider)
	var billing_provider string
	err := row.Scan(&billing_provider)
	return billing_provider, err
}
